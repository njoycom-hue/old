...
- **기억력 카드 짝맞추기**: 같은 그림 카드를 찾는 기억력 게임. 한 판에 5단계로, 단계가 오를수록
  카드 쌍이 늘고(6→10쌍) 미리 보여주는 시간은 짧아집니다(5초→3초, 2단계마다 1초씩)
- **순서 기억하기**: 점점 길어지는 순서를 그대로 따라 누르는 게임 (사이먼 게임 방식). 라운드가
  오를수록 불빛 속도가 조금씩 빨라지되, 최소 속도 아래로는 내려가지 않습니다
- **빠른 암산**: 덧셈/뺄셈 4지선다 10문제
- **다른 것 찾기**: 같은 범주가 아닌 낱말을 찾는 인지 훈련 10문제

두 난이도 곡선 모두 "노인 사용자"를 기준으로 완만하게 설계했습니다 — 급격히 어려워지지 않고
여러 단계/라운드에 걸쳐 천천히 올라갑니다.
...
- 빠른 암산/다른 것 찾기에도 라운드가 오를수록 어려워지는 난이도 곡선 적용
