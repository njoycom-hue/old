...
밋밋한 폼(form) 느낌을 벗어나기 위해, Peak/Lumosity 같은 두뇌트레이닝 앱과 캐주얼 모바일 게임의
UI 관행(청키한 클레이모피즘 버튼, 비비드한 그라데이션, 탄력있는 마이크로 인터랙션)을 벤치마킹해
반영했습니다.

- **입체감 있는 버튼**: 평평한 Material 버튼 대신, 눌렀을 때 아래로 "쑥" 들어가는 3D 버튼
  (홈 화면의 게임 카드를 제외한 모든 버튼)
- **게임별 그라데이션**: 게임 4종이 각자 다른 비비드한 그라데이션 색을 가져서 홈 화면이
  카드 목록이 아니라 서로 다른 "세계"처럼 보이도록 구성. 아이콘은 원형 배지 안에 배치
- **바운스 애니메이션**: 업적 달성 배너, 신기록 달성 시 뜨는 축하 이모지가 스프링 물리 기반으로
  통통 튀며 등장
